...
public class CollectionUtilities {
	...
	public static <T, U> U foldRight_v2(List<T> ts, U identity,
			Generic_Functional_Intrf<T, Generic_Functional_Intrf<U, U>> func) {
		return ts.isEmpty() ? identity : func.apply(head(ts)).apply(foldRight_v2(tail(ts), identity, func));
	}
	...
	public static <T> List<T> iterate(T seed, Generic_Functional_Intrf<T, T> f, int n) {
	...
	public static <T> List<T> iterate(T seed, Generic_Functional_Intrf<T, T> f, Generic_Functional_Intrf<T, Boolean> p) {
		List<T> result = new ArrayList<>();
		T temp = seed;
		while (p.apply(temp)) {
			result.add(temp);
